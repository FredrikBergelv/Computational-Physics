...
def Convergence_plot(size, number, steps, save=False):
    ...
    for i in range(steps):
        ...
        if grid_flatt[moving_cow] % 2 == 0:  # Ifvlaue is 2: kill
            grid_flatt[moving_cow] = 0   # Kill cow
            if np.all(grid_flatt == 0):  # Check if all cows are gone
                break
        ...
        # Find and calculate the density of cows
        density = (len(np.where(grid_flatt != 0)[0])-1)/size**2
        density_list.append(density)
    
    def f(x,A): # Fitting a exponential function
        rho0 = number/(size**2)
        return rho0*np.e**(-A*x)
    
    # Make a fit
    fit = curve_fit(f, list(range(len(density_list))), density_list)
    x_val = np.linspace(0,max(list(range(len(density_list)))), 1000)
    
    plt.plot(x_val, f(x_val, fit[0][0]), c='r', 
             label = rf'$\rho_{{cow}}$ = {number/(size**2):.2f} exp(-{fit[0][0]:.4f}Steps) ')
    
    plt.plot(list(range(len(density_list))), density_list, label='The simulated data')
    plt.grid(True)
    plt.xlabel('Steps')
    plt.ylabel(r'$\rho_{{cow}}$ [cows/pixels$^2$]')
    plt.title('Plot of Cow Censity per Step')
    plt.legend()
    plt.show()
...
Convergence_plot(size=20, number=200, steps=5000, save=True)
